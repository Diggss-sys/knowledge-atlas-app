...
> **2026-07-07 — R0 + R1 landed (Paco's lane, overnight).** The pure-C# experiment core is built and tested in `unity/Assets/RoomGen/Runtime/Runner/` (branch `paco/lighting-l0-l4`, unmerged): **R0** row writer (`ResponseRow`, `ResponseCsv` canonical 17-col RFC-4180, `ResponseWriter` validate→CSV+JSONL, `ResponseLogValidator` reusing `JsonSchemaLite`) — reproduces the 2 golden valid rows and refuses all 5 invalid ones. **R1** seeded ordering (`TrialSequencer`: fixed/seeded_shuffle/latin_square, balanced, counterbalanced choice sides — System.Random, never UnityEngine.Random), `StudyGate` (refuse unless published+validated), `ScriptedSession` + `StudyRunner` (consumes a full study doc → validated CSV, both rating & choice). Sample study at `Resources/RoomGen/Examples/ceiling-study.json`. **Remaining for R2+: the 4 participant screens (UI/Runner/), driving real rooms via LocalChannel + walk + fade + dwell timers, and the queued POST (R3).** Gotcha: the EditMode test asmdef can't see Newtonsoft — keep JObject inside Runtime, hand tests plain types.
