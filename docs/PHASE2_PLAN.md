...
**North-star use case (Diego, 2026-06-11):** subject in a VR headset; a second student edits the room around them live via a friendly slider UI; fully parametric. Feasibility confirmed → [docs/VR_LIVE_EDITING.md](VR_LIVE_EDITING.md): doable, reuses the RoomSpec contract unchanged (the JS↔Unity bridge generalizes to a network bridge — same `ApplySpec(json)`). VR = native Unity OpenXR arm (best fidelity); WebGL = authoring + interactive web-3D participant arm (no 2D); both share RoomSpec + C# generation. Main risk = comfort when geometry changes around an immersed user → default to teleport-between-variants (Kirsh's own idea) or fade-swap for geometry; live-morph lighting/material/furniture freely. VR is the platform's end goal, not an optional Phase 5.

**Editor = Unity, locked (2026-06-11, supersedes the HTML web-UI workstream).** The operator's slider UI is built in **Unity (UI Toolkit)**, not HTML. Rooms generated + rendered in Unity (the "good rooms from the source"). Best-case realization = **single Unity PCVR app, no networking**: subject in a tethered headset, operator at the same PC's monitor, slider → in-memory RoomSpec → `RoomBuilder.BuildFromSpec` rebuilds the one shared scene → both views update instantly, full PC-GPU fidelity ([VR_LIVE_EDITING.md](VR_LIVE_EDITING.md) "Best architecture"). Networked two-client (Netcode/Photon) is the fallback only for standalone/untethered or two-machine setups. Consequence: the old HTML `WEBUI_WORKSTREAM` is replaced by a **Unity UI workstream**; RoomSpec stays the contract (save/load/library/validator), but the live transport is **in-process**, not a web bridge. Cloudflare/SDSC = optional offline room-library storage, never in the live loop.
...
- **`handoffs/CLOUDFLARE_WORKSTREAM.md`** — start from old repo's `cloudflare/v2/room_api_worker.js` pattern (auth, hashing, MIME — in the predecessor repo `1michaelbongiorno/cogs160track3v2` under `cloudflare/v2/`); new resources `atlas-room-api` / `atlas-room-assets` / `atlas_rooms` D1; implement ROOM_API.md + schema.sql; **JS validator module `web/shared/pair_diff.js`** (flatten/diff/coverage port of validate_pair.py) shared by Worker and editor, must pass `diff_vectors.json`; deploy runbook (wrangler login is the only human-OAuth step); DoD: miniflare/`wrangler dev` test script exercising every endpoint incl. 422 on the confounded fixture and a batch POST of fixture response rows.
- **`handoffs/UNITY_UI_WORKSTREAM.md`** (replaces the old HTML `WEBUI_WORKSTREAM`, locked 2026-06-11) — the operator UI is **Unity UI Toolkit**, in the same Unity project as RoomBuilder. Slider/toggle panel bound to RoomSpec fields within preset ranges → calls `RoomBuilder.BuildFromSpec` directly (in-process, single-app best case) or sends RoomSpec over Netcode/Photon (networked fallback) → **live diff panel** (C# port of the validator, or reuse the Python logic via the shared `diff_vectors.json` fixtures) → save room / save pair / publish study to the Cloudflare room library; publish blocked on validator violations. The participant **runner** (trial loop, seeded ordering, tasks, response logging) is also Unity now (was the HTML runner) — or kept minimal/native. DoD: operator drags a slider → room rebuilds live in the (PCVR) scene; confounded pair blocked from publish; a fake-participant session writes response rows. **Note:** Cloudflare D1/Worker is now storage + response sink only, not the live transport.
...
4. Open question to Kirsh still pending (response types; web-3D vs VR as primary outcome — project is 3D-only, no 2D) — absorbed by the task-registry design either way.
...
- **Professor proposal drafted** → [docs/PROPOSAL.md](PROPOSAL.md): cited science (Meyers-Levy ceiling, Vartanian curvature, Ulrich/Kaplan windows), realism-as-measured-score (IPQ/SUS/Witmer-Singer + pilot threshold), tasks+power (within-subjects N≈24), modality verdict (interactive web-3D valid primary for cognition per evidence; native VR immersive arm; 3D-only, no 2D), landscape gap (jsPsych/Gorilla/MPI), 5 questions for Kirsh. Internal technical stress-test → [docs/TECH_FEASIBILITY.md](TECH_FEASIBILITY.md): stage-by-stage go/no-go + risk register; verdict buildable w/ native fallback. Remaining: get Kirsh's answers to the 5 questions; pick flagship study.
